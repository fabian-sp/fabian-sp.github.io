...
* We explain what decoupling of weight decay and learning rate means, and why it is important in practice.
...
*Disclaimer:* I have previously written a blog post [2] on the weight decay mechanism of <tt>AdamW</tt>, and how it can be seen as a proximal version of <tt>Adam</tt> (the blog post is based on the paper by Zhuang et al [4]). This post will re-use some of the figures and contents. In fact, I stumbled upon the central question of this blog post during writing back then. 
...
Let us denote the <tt>Adam</tt> preconditioner by $D_t = \mathrm{diag}(\epsilon + \sqrt{\hat v_t})$.

The way that <tt>AdamW</tt> was proposed originally in the paper by Loshchilov and Hutter [1] is 
$$
w_{t+1} = (1-\lambda\eta_t)w_t - \alpha_t D_t^{-1}\hat m_t. \nonumber
$$


Let us call this method <tt>AdamW-LH</tt>.

In [Pytorch](https://pytorch.org/docs/stable/generated/torch.optim.AdamW.html), the method is implemented slightly differently:
$$
w_{t+1} = (1-\lambda\alpha_t)w_t - \alpha_t D_t^{-1}\hat m_t. \nonumber
$$


Let us call this method <tt>AdamW-PT</tt>. Note that the only difference consists in the coefficient $1-\lambda\alpha_t= 1-\lambda\alpha\eta_t$ instead of $1-\lambda\eta_t$. While this seems like trivia at first sight (one could easily reparametrize $\lambda$ by $\lambda \alpha$), we will show that it has an important practical implication on tuning.
...
While this claim is supported by experiments in the paper, we will show next an example where there is no decoupling when using <tt>AdamW</tt> from Pytorch. The reason for this, as we will show, is exactly the implementation subtlety we described in the previous section.

### A simple experiment

The experiment is as follows: we solve a ridge regression problem for some synthetic data $A \in \mathbb{R}^{n \times d},~b\in \mathbb{R}^{n}$ with $n=200,~d=1000$. Hence $\ell$ is the squared loss, given by $\ell(w) = \|Aw-b\|^2$.
...
**Conclusion 1:** Using the Pytorch implementation <tt>AdamW-PT</tt>, the parameters choices for $\alpha$ and $\lambda$ are not decoupled (for a simple ridge regression example). However, the originally proposed method <tt>AdamW-LH</tt> indeed shows decoupling.
...
Strategy (S1) is slightly easier to code; my conjecture (I don't have a proof) is that (S1) is also employed more often than (S2) in practice.
...
The main insight here is: for the middle and right ones, it would be much easier to obtain a low final loss, as for the left one. This is important when the sweep has only few trials due to, for example, high computational costs for a single run.
...
* This blog post only covers a ridge regression problem, and one might argue that the results could be different for other tasks. However, what this experiment shows is that there certainly is no decoupling for <tt>AdamW-PT</tt> for one of the simplest possible problems. I also observed good performance of the (S2) strategy for <tt>AdamW-PT</tt> when training a vision transfomer on Imagenet (with the `timm` [library](https://github.com/huggingface/pytorch-image-models)).
