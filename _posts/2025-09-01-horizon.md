...
In the paper, we specifically designed a schedule for a two-phase continual learning scenario. Our proposed schedule is piecewise constant in each phase (combined with warmup and linear cooldown in the end). See the green lines in Figure 1, left below.

In this blog post, we ask ourselves the question: what happens with more than two phases? What is the limiting behaviour?
...
**Disclaimer:** in contrast to the paper, where we tested these schedules on actual LLM pretraining tasks, here we will only look at their theoretical performance in terms of a worst-case suboptimality bound.
...
Our solution is to adapt the schedule \\((\eta_t)_t\\) in between \\(T_1\\) and \\(T_2\\), so that we can keep the base learning rate the same. Specifically, we construct a piecewise constant schedule which decreases once after \\(T_1\\). This piecewise constant schedule is combined with a WSD-like approach (warmup-stable-decay), which uses a short warmup and linear cooldown to zero over the final 20% of steps.

 The decreasing factor can be computed from convex optimization theory (for free). In our experiments, we set \\(T_1=50k\\) and tried both \\(T_2\in \{100k,200k\}\\). Figure 1 below shows that this adapted schedule leads to an improvement in LLM pretraining, compared to keeping the schedule unmodified for the longer horizon.[^1]

<p align="center">
    <img src="/images/blog/horizon/paper_schedule.png" width="347" />
    <img src="/images/blog/horizon/paper_loss.png" width="350" />
    <br>
    <em> <b>Figure 1:</b> When extending the training horizon T from 50k to 100k or 200k steps, the piecewise constant schedule (left, green) constructed from theory achieves loss improvement for LLM pretraining compared to baseline (grey).  </em>
</p>

This brings us to the **main question** of this blog post: what happens if we go beyond two periods of training? What is the *limiting schedule* if we do the same sequentially for training horizons \\(T_1 < T_2<\cdots<T_n\\) and \\(n\to +\infty\\)?


## Setup (Part II)

Let \\(n\in \mathbb{N}\\) and \\(\Delta t\in \mathbb{N}\\). The starting point is indexed with \\(t=1\\).[^2]
Assume that we train in total for \\(n\\) periods, defined by the checkpoints \\(T_k = 1+  k \cdot \Delta t\\) with \\(k=1,\ldots,n\\).
...
1) **Piecewise constant**. This schedule is constructed sequentially: at each checkpoint \\(T_k\\), we decide on the next period up to \\(T_{k+1}\\). During each period the schedule is constant (but at decreasing values). As in the paper, we compute the decreasing factor based on the theory, in order to keep the schedule as close to theoretically optimal as possible.
...
Notice that for linear-decay we construct a different schedule for each checkpoint/training horizon \\(T_k\\) (\\(k=1,\ldots,n\\)); in contrast to this, we construct one single **piecewise constant** and **sqrt** schedule that covers \\([1,T_n]\\) (one could call these *"anytime"* schedules). 
...
<p align="center">
    ...
    <br>
    <em> <b>Figure 2:</b> Theoretical bound for SGD with arbitrary schedule in convex, nonsmooth optimization. </em>
</p>
...
This is not surprising: linear-decay is known to achieve the best worst-case bound (Defazio et al. 2023), and as it can plan for the entire horizon, it should also yield better performance.
...
* Compared to the ideal linear-decay schedule, the drawback of our piecewise constant strategy grows with training length. Compared to the no-lookahead sqrt-schedule, even rather short lookahead still yields a noticeable advantage.
...
<p align="center">
    <img src="/images/blog/own/xxx.jpg" width="800"/>
</p>
...
Shamir, O. and Zhang, T. Stochastic gradient descent for non-smooth optimization: Convergence results and optimal averaging schemes. *International Conference on Machine Learning*, 2013. [[link]](https://proceedings.mlr.press/v28/shamir13.pdf)
